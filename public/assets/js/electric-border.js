 ...
 * Animated electric border effect using canvas and Perlin noise
 */
...
class ElectricBorder {
    ...
    `;
        ...
        this.element.style.setProperty('--electric-border-radius', `${this.options.borderRadius}px`);
...
// Auto-initialize on elements with data-electric-border attribute
document.addEventListener('DOMContentLoaded', () => {
    const elements = document.querySelectorAll('[data-electric-border]');
    elements.forEach(el => {
        const color = el.dataset.electricBorderColor || '#5227FF';
        const speed = parseFloat(el.dataset.electricBorderSpeed) || 1;
        const chaos = parseFloat(el.dataset.electricBorderChaos) || 0.12;
        const borderRadius = parseFloat(el.dataset.electricBorderRadius) || 24;

        new ElectricBorder(el, { color, speed, chaos, borderRadius });
    });
});
